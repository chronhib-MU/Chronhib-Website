...
export let tables = {
  names: ['text', 'sentences', 'lemmata', 'morphology'],
  text: {
    headers: ['ID', 'Text ID', 'Revised Title'],
    bodies: ['ID_unique_number', 'Text_ID', 'Revised_title'],
    data: []
  },
  sentences: {
    headers: ['ID', 'Text ID', 'Textual Unit ID', 'Textual Unit', ' Locus'],
    bodies: ['ID_unique_number', 'TextID', 'Textual_Unit_ID', 'Textual_Unit', 'Locus1'],
    data: []
  },
  morphology: {
    headers: [
...
export const updateData = updatedData => {
  Object.keys(updatedData).forEach(key => {
    tables[key].data = updatedData[key];
  });
  console.log('updateData:', tables);
  return tables;
};
